...
export default function App() {
  ...
  function fetchSheets(onDone, onError) {
  ...
  useEffect(() => {
    fetchSheets(
      (data) => {
        if (data.bahan && data.bahan.length > 0) setBahan(data.bahan.map(b => ({ ...b, unit: "g" })));
        if (data.pkg) setPkg(data.pkg);
        setLastSync(new Date());
        setSyncError(false);
        setLoading(false);
      },
      () => { setSyncError(true); setLoading(false); }
    );
  }, []);
  ...
  return (
    ...
    <div style={{ maxWidth:560, margin:"0 auto", paddingBottom:80, minHeight:"100vh", boxShadow:"0 0 60px #00000066" }}>
